...
class RSSParser {
	...
	public $cache_dir 		= './slacms/cache/'; 	// Cache directory
	...
	function parse()
	{
            ...
            if($CI->Sla_model->is_url_exist($this->feed_uri) !== FALSE){
		// Are we caching?
	...
	*/
	...
	function runFeedBackend($num)
	{
            ...
            if (!$CI->cache->get('backend_rssfeed_news')) {
                ...
                if($CI->Sla_model->load_config()->pagecache_time == 0){
                    $cache_time = 1;
                }else{
                    $cache_time = $CI->Sla_model->load_config()->pagecache_time;
                }
