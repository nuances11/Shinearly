...
class Sla_admin_model extends CI_Model{
    ...
    public function load_config(){
        return $this->Sla_model->load_config();
    }
    ...
    public function getLatestVersion($xml_url = ''){
        if(!$xml_url){
            $xml_url = $this->config->item('sla_chkverxmlurl_main');
        }
        $xml_url_bak = $this->config->item('sla_chkverxmlurl_backup');
        if($this->Sla_model->is_url_exist($xml_url) !== FALSE){
            $xml = @simplexml_load_file($xml_url);
        }else if($this->Sla_model->is_url_exist($xml_url) === FALSE && $this->Sla_model->is_url_exist($xml_url_bak) !== FALSE){
        ...
        if($xml !== FALSE){
            ...
            $xml_cur = $this->Sla_model->getVersion();
    ...
    public function chkVerUpdate($cur_ver, $xml_url = ''){
        ...
        if($last_ver){
            return $this->Sla_model->chkVerUpdate($cur_ver, $last_ver);
        }else{
            return FALSE;
        }
    }
    ...
    public function findNextVersion($cur_txt, $xml_url = ''){
        $last_ver = $this->getLatestVersion($xml_url);
        if($last_ver){
            return $this->Sla_model->findNextVersion($cur_txt, $last_ver);
        }else{
            return FALSE;
        }
    }
    ...
    public function countTable($table, $search_sql = ''){
        ...
        return $this->Sla_model->countData($table, $search_sql);
    }
    ...
    public function getIndexData($table, $limit = 0, $offset = 0, $orderby = '', $sort = '', $search_sql = '', $groupby = '', $join_db = '', $join_where = '', $join_type = ''){
        ...
        $count = $this->Sla_model->countData($table, $search_sql, $groupby, $orderby, $sort, $join_db, $join_where, $join_type);
        $this->db->select('*');
    ...
    public function getUser($id, $type = ''){
        ...
        $rows = $this->Sla_model->getValue('*', 'user_admin', $sql_where, '', 1);
        if($rows !== FALSE){
            return $rows;
        }else{
            return FALSE;
    ...
    public function getUserEmail($id){
        ...
        $rows = $this->Sla_model->getValue('email', 'user_admin', "user_admin_id", $id, 1);
        if($rows !== FALSE){
            return $rows->email;
        }else{
            return FALSE;
     ...
     * Function for create the new user
     */
    public function createStudent($student = FALSE){
        // Create the user account
        if($this->input->post('active')){
            $active = $this->input->post('active', TRUE);
        }else{
     ...
     * Function for create the new user
     */
    public function createTeacher($teacher = FALSE){
        // Create the user account
        if($this->input->post('active')){
            $active = $this->input->post('active', TRUE);
        }else{
    ...
    public function updateUser($id){
        $query = $this->Sla_model->chkPassword($this->session->userdata('admin_email'), $this->input->post('cur_password', TRUE), "user_type != 'member'");
        if($query['num_rows'] !== 0){
            ...
            if($this->input->post('password') != ''){
                $this->db->set('password', $this->Sla_model->pwdEncypt($this->input->post('password', TRUE)), TRUE);
            ...
            if($id != 1 && $this->session->userdata('user_admin_id') != $id){
                ...
                $count = $this->Sla_model->countData('user_to_group', $data);
    ...
    public function sessionLoginChk(){
        ...
        if($this->session->userdata('session_id')){
            $numcount = $this->Sla_model->countData('user_admin', "session_id = '".$this->session->userdata('session_id')."' AND email = '".$this->session->userdata('admin_email')."'");
     ...
     *
     */
    public function slaCopyright(){
        /* Please do not remove or change this function */
        $sla_copyright = '<br><span class="copyright">Powered by <a href="https://www.slacms.com" target="_blank" style="color: gray;">SLA CMS</a> Version '.$this->Sla_model->getVersion().'</span>';
        return $sla_copyright;
    }
    ...
    public function coreMetatags($desc_txt, $more_metatags = ''){
        $meta = array(
            ...
            array('name' => 'author', 'content' => 'SHINEARLY'),
            array('name' => 'generator', 'content' => 'SLA CMS | Version '.$this->Sla_model->getVersion()),
    ...
    public function getAllMenu($drop_page_menu_id = 0, $lang = '', $position = 0){
        return $this->Sla_model->main_menu($drop_page_menu_id, $lang, $position, TRUE);
    }
    ...
    public function findLangDataUpdate($lang_iso){
        ...
        if($query->num_rows() !== 0){
            foreach($query->result() as $rows){
                $this->db->set('lang_iso', $this->Sla_model->getDefualtLang(), TRUE);
                $this->db->set('timestamp_update', 'NOW()', FALSE);
    ...
    public function insertLang(){
        $arrange = $this->Sla_model->getLastID('lang_iso', 'arrange');
        ($this->input->post('active')) ? $active = $this->input->post('active', TRUE) : $active = 0;
        $data = array(
    ...
    public function updateLang($id){
        $old_lang = $this->Sla_model->getValue('lang_iso', 'lang_iso', 'lang_iso_id', $id, 1);
        $this->load->dbforge();
    ...
    public function syncLabelLang(){
        $this->load->dbforge();
        $lang = $this->Sla_model->getValueArray('lang_iso', 'lang_iso', "lang_iso != ''", '');
    ...
    public function chkPageName($page_name_input){
        if($page_name_input == 'assets' || $page_name_input == 'slacms' ||
    ...
    public function insertPage(){
        ...
        ($this->input->post('active')) ? $active = $this->input->post('active', TRUE) : $active = 0;
        $page_url = $this->Sla_model->rw_link($page_name_input);
    ...
    public function updatePage($id){
        ...
        $this->db->update('pages');
        $this->Sla_model->clear_uri_cache($this->config->item('base_url').urldecode($page_url));
        $this->Sla_model->clear_file_cache('file_'.$this->Sla_model->encodeURL($page_url));
    }
    ...
    public function chkPluginActive($plugin_config_filename){
        if($plugin_config_filename){
            $status = $this->Sla_model->getValue('plugin_active', 'plugin_manager', "plugin_config_filename", $plugin_config_filename, 1);
    ...
    public function updateWidget($id){
        ...
        $this->Sla_model->clear_file_cache('widget_'.$this->Sla_model->encodeURL($this->input->post("widget_name", TRUE)));
    }
    ...
    public function saveWhiteIP(){
        ...
        if($this->Sla_model->chkBFwhitelistIP($ip_address) === FALSE && !empty($ip_address)){
            $data = array(
    ...
    public function saveBlackIP(){
        $ip_address = $this->input->post('ip_address', TRUE);
        if($this->Sla_model->chkBFblacklistIP($ip_address) === FALSE && !empty($ip_address)){
            $data = array(
                'ip_address' => $ip_address,
                'note' => $this->input->post('note', TRUE),
            );
    ...
    public function setMaintenance() {
        ...
        $this->db->update('settings', $data);
        $this->db->cache_delete_all();
        $this->Sla_model->clear_file_cache('config');
        $this->Sla_model->clear_file_cache('topmenu_*', TRUE);
    }
    ...
    public function getSaveDraftJS() {
        ...
        $(document).ready(function () {
            $("#save_draft").click(function () {
                ...
                var urlpost = "'.$this->Sla_model->base_link().'/admin/admin/saveDraft'.'";
                $.ajax({
                    url: urlpost,
                    type: "POST",
                    data: fields,
    ...
    public function makePrivateKey() {
        ...
        $private_key = sha1($row->default_email . '|' . $this->Sla_model->base_link() . '|' . $user->password . '|slacms|' . $gmdate);
    ...
    public function getPluginXML($xml_url = ''){
        ...
        if (!$this->cache->get('plugin_list_xml')) {
            if(!$xml_url) $xml_url = $this->config->item('sla_pluginxmlurl_main');
            $xml_url_bak = $this->config->item('sla_pluginxmlurl_backup');
            if($this->Sla_model->is_url_exist($xml_url) !== FALSE){
    ...
    public function chkPluginInst($plugin_config_filename){
        $count = $this->Sla_model->countData('plugin_manager', "plugin_config_filename = '".$plugin_config_filename."'");
        if($count != 0){
            return TRUE;
        }else{
            return FALSE;
