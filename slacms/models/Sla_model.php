...
class Sla_model extends CI_Model {
    ...
    public function getVersion($version_test = '') {
        $con_version = $this->config->item('sla_version'); /* For CMS Version */
        $con_release = $this->config->item('sla_release'); /* For release or beta */
        $version = '';
        if($version_test) {
     ...
     * Please do not remove or change the fuction $this->Sla_admin_model->slaCopyright()
     *
     */
    public function slaCopyright() {
        ...
        $html = '<span class="copyright">' . str_replace(' %YEAR ', ' ' . date('Y') . ' ', $row->site_footer) . '</span>
                <small style="color:gray;">' . $this->Sla_admin_model->slaCopyright() . '</small>'; /* Please do not remove or change the fuction $this->Sla_admin_model->slaCopyright() */
        return $html;
    }
    ...
    public function coreCss($more_css = '', $more_include = TRUE) {
        ...
        $core_css.= '<link href="' . $this->Sla_model->base_link().'/'. 'corecss.css" rel="stylesheet" type="text/css" />' . "\n";
    ...
    public function coreJs($more_js = '', $more_include = TRUE) {
        ...
        $core_js = '<script type="text/javascript" src="' . $this->Sla_model->base_link().'/'. 'corejs.js"></script>';
    ...
    public function coreMetatags($desc_txt, $keywords, $title, $img_path = '', $more_meta = '') {
        ...
        $meta = array(
            ...
            array('name' => 'og:url', 'content' => $this->Sla_model->base_link(). '/' . $this->uri->uri_string(), 'type' => 'property'),
    ...
    public function addBannerToHTML($content, $id) {
        ...
        if (!$this->cache->get('banner_'.$id)) {
            ...
            if ($getBanner !== FALSE) {
                ...
                $html = '<a target="_blank" href="' . $this->Sla_model->base_link(). '/banner/' . $getBanner->banner_mgt_id . '" title="' . $getBanner->name . '"'.$nofol.'><img class="lazy img-responsive img-thumbnail" data-src="' . $img . '" alt="' . $getBanner->name . '"' . $width . $height . '"></a>';
    ...
    public function addFrmToHtml($content, $frm_name, $status = '') { /* Add the form in content */
        ...
        if ($form_data && $form_data !== FALSE) {
            ...
            $html = $sts_msg;
            $action_url = $this->Sla_model->base_link(). '/formsaction/' . $form_data->form_main_id;
            ...
            $cururl = str_replace($this->Sla_model->base_link(). '/', '', current_url());
            $totSegments = $this->uri->total_segments();
            if (is_numeric($this->uri->segment($totSegments))) {
                $cururl = str_replace('/'.$this->uri->segment($totSegments), '', $cururl);
            }
            $sess_data = array('slafrm_cururl' => $cururl);
    ...
    public function showCaptcha() {
        ...
        if ($config->googlecapt_active && $config->googlecapt_sitekey && $config->googlecapt_secretkey) {
            ...
            if($this->uri->segment(1) == 'admin'){
                if($this->Sla_admin_model->getLang()){
                    $hl = '?hl='.$this->Sla_admin_model->getLangISOfromName($this->Sla_admin_model->getLang());
                }
            }else{
    ...
    public function login($email, $password, $backend = FALSE) {
        if ($this->Sla_model->chkCaptchaRes() == '') {
    ...
    public function studentLogin($email, $password) {
                ...
                if ($return['num_rows'] !== 0 && !empty($return['row'])) {
                    $rows = $return['row'];
                    $session_id = session_id();
                    $this->db->set('session_id', $session_id, TRUE);
                    $this->db->where('user_admin_id', $rows->user_admin_id);
        ...
        if ($query['num_rows'] !== 0) {
            ...
            if ($this->input->post('del_file')) {
                ...
                if ($_FILES['file_upload']['type'] == 'image/png' || $_FILES['file_upload']['type'] == 'image/jpg' || $_FILES['file_upload']['type'] == 'image/jpeg' || $_FILES['file_upload']['type'] == 'image/gif') {
                    ...
                    $upload_file = $this->Sla_admin_model->file_upload($file_f, $file_name, $this->input->post('picture', TRUE), $uploaddir, $photo_id, $paramiter);
                }
            }
    ...
    public function chkSched($sched){
        if ($sched == '1') {
            return 'FREE';
        }elseif ($sched == '2') {
            return 'FIXED';
    ...
    public function get_schedule($teacher_id){
        return $this->db->get_where([
            'instructor_id' => $teacher_id
        ]);
    }

    public function check_schedules($slot, $schedules){
