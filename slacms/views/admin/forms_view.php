...
<div class="row">
    <div class="col-lg-12 col-md-12">
        <div class="h2 sub-header"><?php echo $form_name?>  <a role="button" href="<?php echo $this->sla_referrer->getIndex()?>" class="btn btn-default btn-sm"><i class="glyphicon glyphicon-arrow-left"></i> <?php echo  $this->lang->line('btn_back') ?></a></div>
                ...
                <tbody>
                    ...
                    <?php } else { ?>
                        <?php foreach ($post_rs as $u) {
                            ...
                            echo '<td class="text-center" style="vertical-align:middle;"><a role="button" class="btn btn-danger btn-sm" role="button" onclick="return confirm(\''.$this->lang->line('forms_delete_msg').'\')" href="'.$this->Sla_model->base_link().'/admin/forms/view/'.$this->uri->segment(4).'/delete/' . $u['form_'.$form_name.'_id'] . '"><i class="glyphicon glyphicon-remove"></i> '.$this->lang->line('btn_delete').'</a></td>';
                            echo '</tr>';
                        } ?>
                    <?php } ?>
                </tbody>
