...
<div class="row">
    <div class="col-lg-12 col-md-12">
        <div class="h2 sub-header"><a role="button" href="<?php echo ($this->uri->segment(5))?$this->sla_referrer->getIndex('view'):$this->sla_referrer->getIndex()?>" class="btn btn-default btn-sm"><i class="glyphicon glyphicon-arrow-left"></i> <?php echo  $this->lang->line('btn_back') ?></a></div>
                    ...
                    <?php if($this->uri->segment(5)){ 
                        if ($bannerstat !== FALSE) {
                            foreach ($bannerstat as $u) { 
                                ...
                                $count_d = $this->Sla_model->countData('banner_statistic', $search_arr_d); ?>
                    ...
                    <?php }else{ ?>
                        <?php if ($year !== FALSE) { ?>
                            <?php foreach ($year as $u) { 
                                ...
                                $month = $this->Sla_model->getValueArray("MONTHNAME(STR_TO_DATE(MONTH(timestamp_create), '%m')) AS banner_month_name, MONTH(timestamp_create) AS banner_month", 'banner_statistic', $search_arr_y, '', 0, 'banner_month', 'DESC', 'banner_month'); 
                                if ($month !== FALSE) { 
                                    foreach ($month as $m) { 
                                        ...
                                        $count_m = $this->Sla_model->countData('banner_statistic', $search_arr_m); ?>
                                        <li><a href="<?php echo $this->Sla_model->base_link().'/admin/banner/view/' . $this->uri->segment(4) . '/' . $u['banner_year'] . '-' . str_pad($m['banner_month'], 2, '0', STR_PAD_LEFT) ; ?>"><?php echo $u['banner_year'].' '.$m['banner_month_name'] ?> <span class="pull-right badge bg-red"><?php echo $count_m ?></span></a></li>
                                    <?php } ?>
                                <?php } ?>
                            <?php } ?>
                        <?php } ?>
