...
class Linkstats extends CI_Controller {
    ...
    public function index() {
        ...
        // Pages variable
        $result_per_page = 20;
        $total_row = $this->Sla_model->countData('link_stat_mgt', $search_arr);
        $num_link = 10;
        $base_url = $this->Sla_model->base_link(). '/admin/linkstats/';
        ...
        $this->template->setSub('linkstats', $this->Sla_admin_model->getIndexData('link_stat_mgt', $result_per_page, $pagination, 'timestamp_create', 'desc', $search_arr));
    ...
    public function insert() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //Validation passed
            //Add the user
            $this->Sla_admin_model->insertLinks();
            //Return to user list
    ...
    public function view() {
        ...
        if($this->uri->segment(4)){
            ...
            $this->load->helper('form');
            $this->load->library('pagination');   
            $getLink = $this->Sla_model->getValue('url', 'link_stat_mgt', 'link_stat_mgt_id', $this->uri->segment(4), 1);
            ...
            // Pages variable
            $result_per_page = 20;
            $total_row = $this->Sla_model->countData('link_statistic', $search_arr);
            $num_link = 10;
            $base_url = $this->Sla_model->base_link(). '/admin/linkstats/view/'.$this->uri->segment(4).'/';
            ...
            //Get users from database
            $this->template->setSub('linkstats', $this->Sla_admin_model->getIndexData('link_statistic', $result_per_page, $pagination, 'link_statistic_id', 'desc', $search_arr));
            $this->template->setSub('total_row',$total_row);
    ...
    public function deleteViewByID() {
        ...
        if(isset($delR)){
            foreach ($delR as $value) {
                if ($value) {
                    $this->Sla_admin_model->removeData('link_statistic', 'link_statistic_id', $value);
                }
            }
        }
        $this->db->cache_delete_all();
    ...
    public function deleteIndexByURL() {
        ...
        if(isset($delR)){
            foreach ($delR as $value) {
                if ($value) {
                    $getLink = $this->Sla_model->getValue('url', 'link_stat_mgt', 'link_stat_mgt_id', $value, 1);
                    $this->Sla_admin_model->removeData('link_statistic', 'link', $getLink->url);
                    $this->Sla_admin_model->removeData('link_stat_mgt', 'link_stat_mgt_id', $value);
                }
            }
        }
        $this->db->cache_delete_all();
