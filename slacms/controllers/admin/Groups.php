...
class Groups extends CI_Controller {
    ...
    public function index() {
        ...
        // Pages variable
        $result_per_page = 20;
        $total_row = $this->Sla_admin_model->countTable('user_groups');
        $num_link = 10;
        $base_url = $this->Sla_model->base_link(). '/admin/groups/';
        ...
        $this->template->setSub('groups', $this->Sla_admin_model->getIndexData('user_groups', $result_per_page, $pagination, 'user_groups_id', 'ASC'));
    ...
    public function add() {
        ...
        $this->template->setSub('b_perms', $this->Sla_auth_model->get_perms_all('backend'));
        $this->template->setSub('f_perms', $this->Sla_auth_model->get_perms_all('frontend'));
        //Load the view
    ...
    public function insert() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //Validation passed
            $this->Sla_auth_model->create_group($this->input->post("name", TRUE), $this->input->post("definition", TRUE), $this->input->post("perms"));
    ...
    public function edit() {
        ...
        if ($this->uri->segment(4)) {
            $group = $this->Sla_model->getValue('*', 'user_groups', 'user_groups_id', $this->uri->segment(4), 1);
            if($group !== FALSE){
    ...
    public function update() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($this->uri->segment(4)){
                //Update the user
                $this->Sla_auth_model->update_group($this->uri->segment(4), $this->input->post("name", TRUE), $this->input->post("definition", TRUE), $this->input->post("perms"));
                //Return to user list
                $this->db->cache_delete_all();
                $this->session->set_flashdata('error_message', '<div class="alert alert-success" role="alert">' . $this->lang->line('success_message_alert') . '</div>');
            }          
    ...
    public function delete() {
        ...
        if ($this->uri->segment(4)) {
            if($this->uri->segment(4) != 1 && $this->uri->segment(4) != 2 && $this->uri->segment(4) != 3 && $this->uri->segment(4) != 4){
                $this->Sla_auth_model->delete_group($this->uri->segment(4));
                $this->db->cache_delete_all();
                $this->session->set_flashdata('error_message', '<div class="alert alert-success" role="alert">' . $this->lang->line('success_message_alert') . '</div>');
            }else{
