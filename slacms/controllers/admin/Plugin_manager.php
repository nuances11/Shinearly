...
class Plugin_manager extends CI_Controller {
    ...
    public function index() {
        ...
        $total_row = $this->Sla_model->countData('plugin_manager');
        
        //Get users from database
        $this->template->setSub('plugin_mgr', $this->Sla_model->getValueArray('*', 'plugin_manager', '', ''));
        $this->template->setSub('total_row', $total_row);
        $this->template->setSub('plugin_list', $this->Sla_admin_model->getPluginXML());
    ...
    public function setstatus() {
        ...
        if ($this->uri->segment(4)) {
            $status = $this->Sla_model->getValue('plugin_active', 'plugin_manager', "plugin_config_filename != '' AND plugin_manager_id = '".$this->uri->segment(4)."'", '', 1);
    ...
    public function install() {
        ...
        if ($this->uri->segment(4) && $this->Sla_admin_model->chkPluginInst($this->uri->segment(4)) === FALSE) {
            $version = $this->Sla_admin_model->pluginLatestVer($this->uri->segment(4));
            $url1 = $this->config->item('sla_plugin_install_server_1') . $this->uri->segment(4)."-install-" . $version . ".zip";
            $url2 = $this->config->item('sla_plugin_install_server_2') . $this->uri->segment(4)."-install-" . $version . ".zip";
            if($this->Sla_model->is_url_exist($url1) !== FALSE){
                $url = &$url1; /* Main Link */
            }else if($this->Sla_model->is_url_exist($url1) === FALSE && $this->Sla_model->is_url_exist($url2) !== FALSE){
                $url = &$url2; /* Backup Link */
            }
            $newfname = FCPATH . basename($url);
            if($this->Sla_model->downloadFile($url, $newfname) !== FALSE){
                if (file_exists($newfname)) {
                    $unzip = @$this->unzip->extract($newfname, FCPATH);
                    if(!empty($unzip)){
                        if (file_exists(FCPATH . 'plugin_sql/install.sql')) {
                            $this->Sla_admin_model->execSqlFile(FCPATH . 'plugin_sql/install.sql');
    ...
    public function upgrade() {
        ...
        if ($this->uri->segment(4) && $this->Sla_admin_model->chkPluginInst($this->uri->segment(4)) !== FALSE) {
            $last_ver = $this->Sla_admin_model->pluginLatestVer($this->uri->segment(4));
            $cur_ver = $this->Sla_model->getPluginConfig($this->uri->segment(4), 'plugin_version');
            if($this->Sla_admin_model->chkPluginUpdate($cur_ver, $last_ver) !== FALSE){
                $nextversion = $this->Sla_admin_model->pluginNextVer($cur_ver, $last_ver);
                $url1 = $this->config->item('sla_plugin_upgrade_server_1') . $this->uri->segment(4)."-upgrade-" . $nextversion . ".zip";
                $url2 = $this->config->item('sla_plugin_upgrade_server_2') . $this->uri->segment(4)."-upgrade-" . $nextversion . ".zip";
                if($this->Sla_model->is_url_exist($url1) !== FALSE){
                    $url = &$url1; /* Main Link */
                }else if($this->Sla_model->is_url_exist($url1) === FALSE && $this->Sla_model->is_url_exist($url2) !== FALSE){
                    $url = &$url2; /* Backup Link */
                ...
                if($this->Sla_model->downloadFile($url, $newfname) !== FALSE){
                    if (file_exists($newfname)) {
                        ...
                        if(!empty($unzip)){
                            if (file_exists(FCPATH . 'upgrade_sql/upgrade.sql')) {
                                $this->Sla_admin_model->execSqlFile(FCPATH . 'upgrade_sql/upgrade.sql');
                    ...
                    if($this->Sla_admin_model->chkPluginUpdate($this->Sla_model->getPluginConfig($this->uri->segment(4), 'plugin_version'), $last_ver) !== FALSE){
