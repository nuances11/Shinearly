...
class Admin extends CI_Controller {
    ...
    public function index() {
        ...
        $this->template->setSub('email_logs', $this->Sla_model->getValueArray('*', 'email_logs', "ip_address != ''", '', 10, 'timestamp_create', 'desc'));
        $this->template->setSub('link_stats', $this->Sla_model->getValueArray('*', 'link_statistic', "ip_address != ''", '', 20, 'timestamp_create', 'desc'));
        $this->template->setSub('total_emaillogs', $this->Sla_model->countData('email_logs'));
        $this->template->setSub('total_linkstats', $this->Sla_model->countData('link_statistic'));
        $this->template->setSub('total_student', $this->Sla_model->countData('user_admin',"user_type = 'student'"));
        //$this->load->library('RSSParser');
        if($this->Sla_model->is_url_exist($this->config->item('sla_backend_feed_url')) !== FALSE){
            $url = $this->config->item('sla_backend_feed_url'); /* Main Link */
        }else{
            $url = $this->config->item('sla_backend_feed_backup_url'); /* Backup Link */
        }
        //$this->rssparser->set_feed_url($url);  /* get feed from SLA CMS Article */
    ...
    public function analytics() {
        ...
        $this->db->cache_on();
        $settings = $this->Sla_admin_model->load_config();
        $this->sla_referrer->setIndex();
    ...
    public function deleteEmailLogs() {
        ...
        if($this->uri->segment(4)) {
            $this->Sla_admin_model->removeData('email_logs', 'email_logs_id', $this->uri->segment(4));
    ...
    public function loginCheck() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $result = $this->Sla_model->login($email, $password, TRUE); /* TRUE for backend login */
            if ($result == 'SUCCESS') {
                $this->Sla_model->saveLogs($email, 'Backend Login Successful!', $result);
                if($this->session->userdata('slablogin_cururl')){
                    redirect($this->session->userdata('slablogin_cururl'), 'refresh');
                }else{
                    redirect($this->Sla_model->base_link().'/admin', 'refresh');
                }
            } else {
                $this->Sla_model->saveLogs($email, 'Backend Login Invalid!', $result);
    ...
    public function logout() {
        $this->Sla_model->logout($this->Sla_model->base_link().'/admin');
    }
    ...
    public function social() {
        ...
        $this->template->setSub('social', $this->Sla_admin_model->getSocial());
    ...
    public function updateSocial() {
        ...
        $this->Sla_admin_model->updateSocial();
        $this->db->cache_delete_all();
        $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
        redirect($this->sla_referrer->getIndex(), 'refresh');
    }
    ...
    public function genSitemap() {
        ...
        admin_helper::is_allowchk('save');
        $this->load->model('Sla_sitemap');
        $this->Sla_sitemap->runSitemap();
        $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
        redirect($this->sla_referrer->getIndex(), 'refresh');
    }
    ...
    public function settings() {
        ...
        $this->template->setSub('sitemaptime', $this->Sla_sitemap->getFileTime());
        $this->template->setSub('settings', $this->Sla_admin_model->load_config());
    ...
    public function updateSettings() {
        ...
        admin_helper::is_allowchk('site settings');
        admin_helper::is_allowchk('save');
        $this->Sla_admin_model->updateSettings();
        $this->db->cache_delete_all();
    ...
    public function uploadIndex() {
        ...
        // Pages variable
        $result_per_page = 20;
        $total_row = $this->Sla_model->countData('upload_file', $search_arr);
        $num_link = 10;
        $base_url = $this->Sla_model->base_link(). '/admin/uploadindex/';

        // Pageination config
        $this->Sla_admin_model->pageSetting($base_url, $total_row, $result_per_page, $num_link);
        ($this->uri->segment(3)) ? $pagination = ($this->uri->segment(3)) : $pagination = 0;
        
        $this->template->setSub('showfile', $this->Sla_admin_model->getIndexData('upload_file', $result_per_page, $pagination, 'timestamp_create', 'desc', $search_arr));
    ...
    public function uploadDownload() {
        ...
        if($this->uri->segment(3)){
            $file = $this->Sla_model->getValue('file_upload', 'upload_file', "upload_file_id", $this->uri->segment(3), 1);
    ...
    public function uploadIndexSave() {
        ...
        if(isset($filedel)){
            ...
            foreach ($filedel as $value) {
                if ($value) {
                    $filename = $this->Sla_model->getValue('file_upload', 'upload_file', 'upload_file_id', $value, 1);
                    if($filename->file_upload){
                        @unlink($path . $filename->file_upload);
                    }
                    $this->Sla_admin_model->removeData('upload_file', 'upload_file_id', $value);
                }
            }
        }
    ...
    public function htmlUpload() {
        ...
        for($i=0; $i<$cpt; $i++) {   
            if($files['files']['name'][$i]){
                ...
                $file_id1 = $this->Sla_admin_model->file_upload($photo, $photo_name, '', $path, $file_id, '', $year);
                if ($file_id1) {
                    $this->Sla_admin_model->insertFileUpload($year, $file_id1);
                }
            }
        }
        $this->db->cache_delete_all();
    ...
    public function tinyMCEfile() {
        ...
        $total_row = $this->Sla_model->countData('upload_file', $search_arr);
        $num_link = 10;
        $base_url = $this->Sla_model->base_link(). '/admin/tinyMCEfile/';

        // Pageination config
        $this->Sla_admin_model->pageSetting($base_url, $total_row, $result_per_page, $num_link);
        ($this->uri->segment(3)) ? $pagination = ($this->uri->segment(3)) : $pagination = 0;
        $data['showfile'] = $this->Sla_admin_model->getIndexData('upload_file', $result_per_page, $pagination, 'timestamp_create', 'desc', $search_arr);
        $data['total_row'] = $total_row;
        $data['core_css'] = $this->Sla_admin_model->coreCss();
        $data['core_js'] = $this->Sla_admin_model->coreJs();
    ...
    public function saveDraft() {
        ...
        if(!empty($_POST) && is_array($_POST)){
            ...
            foreach ($_POST as $key => $value) {
                if($key && $value && $key != 'csrf_sla' && $key != 'current_url'){
                    if($key == 'content'){
            ...
            if(!empty($formarr) && !empty($referrer)){
                ...
                $chk_draft = $this->Sla_model->countData('save_formdraft', "form_url = '".$referrer."'");
                if($chk_draft > 0){
