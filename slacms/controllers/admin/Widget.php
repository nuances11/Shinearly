...
class Widget extends CI_Controller {
    ...
    public function index() {
        ...
        // Pages variable
        $result_per_page = 20;
        $total_row = $this->Sla_admin_model->countTable('widget_xml');
        $num_link = 10;
        $base_url = $this->Sla_model->base_link(). '/admin/widget/';
        ...
        $this->template->setSub('widget', $this->Sla_admin_model->getIndexData('widget_xml', $result_per_page, $pagination, 'widget_xml_id', 'ASC'));
    ...
    public function insert() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //Validation passed
            //Add the user
            $this->Sla_admin_model->insertWidget();
            //Return to user list
    ...
    public function editWidget() {
        ...
        if ($this->uri->segment(4)) {
            $this->db->cache_on();
            $widget = $this->Sla_model->getValue('*', 'widget_xml', 'widget_xml_id', $this->uri->segment(4), 1);
            if($widget !== FALSE){
    ...
    public function edited() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($this->uri->segment(4)){
                //Update the user
                $this->Sla_admin_model->updateWidget($this->uri->segment(4));
                //Return to user list
                $this->db->cache_delete_all();
                $this->session->set_flashdata('error_message', '<div class="alert alert-success" role="alert">' . $this->lang->line('success_message_alert') . '</div>');
            }          
    ...
    public function delete() {
        ...
        if ($this->uri->segment(4)) {
            //Delete the widget
            $this->Sla_admin_model->removeData('widget_xml', 'widget_xml_id', $this->uri->segment(4));
            $this->db->cache_delete_all();
            $this->Sla_model->clear_file_cache('widget_*', TRUE);
            $this->session->set_flashdata('error_message', '<div class="alert alert-success" role="alert">' . $this->lang->line('success_message_alert') . '</div>');
        }
