...
class Upgrade extends CI_Controller {
    ...
    public function _init() {
        ...
        $this->cur_version = $this->Sla_model->getVersion();
        $this->last_version = $this->Sla_admin_model->getLatestVersion();
    }
    ...
    public function index() {
        ...
        $this->session->unset_userdata('slacms_lastver');
    ...
    public function download() {
        ...
        $lastversion = $this->Sla_admin_model->chkVerUpdate($this->cur_version);
        if ($lastversion !== FALSE) {
            $nextversion = $this->Sla_admin_model->findNextVersion($this->cur_version);
            $url1 = $this->config->item('sla_upgrade_server_1') . "upgrade-to-" . $nextversion . ".zip";
            $url2 = $this->config->item('sla_upgrade_server_2') . "upgrade-to-" . $nextversion . ".zip";
            ...
            if($this->Sla_model->downloadFile($url, $newfname) !== FALSE){
                $this->Sla_admin_model->setMaintenance();
                ...
                if($this->Sla_admin_model->chkVerUpdate($this->Sla_model->getVersion()) !== FALSE){
                    redirect('/admin/upgrade/download', 'refresh');
                }else{
                    $this->Sla_admin_model->unsetMaintenance();
    ...
    public function fileBackup() {
        ...
        foreach (directory_map('templates/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if (($t != "index.html" && $t != ".htaccess") && ($t != "admin") && (strpos($t, 'admin') === false) && ($t != "sladefault") && (strpos($t, 'sladefault') === false)) {
                    $this->zip->read_dir('templates/' . $t . '/');
                }
            }
        }
        foreach (directory_map('slacms/views/templates/', 1) as $t) {
            if (!is_dir($t)) {
                $t = str_replace("\\", "", $t);
                $t = str_replace("/", "", $t);
                if (($t != "index.html") && ($t != "admin") && (strpos($t, 'admin') === false) && ($t != "sladefault") && (strpos($t, 'sladefault') === false)) {
                    $this->zip->read_dir('slacms/views/templates/' . $t . '/');
                }
            }
        }
    ...
    public function clearAllSession() {
        ...
        $this->Sla_model->clear_all_session();
        @array_map('unlink', glob(FCPATH.'/'.EMAIL_DOMAIN . '_*'));
        $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
    ...
    public function clearAllErrLog() {
        ...
        admin_helper::is_allowchk('maintenance');
        admin_helper::is_allowchk('delete');
        $this->Sla_model->clear_all_error_log();
        @array_map('unlink', glob(FCPATH.'/'.EMAIL_DOMAIN . '_*'));
        $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
        redirect('admin/upgrade', 'refresh');
    }
