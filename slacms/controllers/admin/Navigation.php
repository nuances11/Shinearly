<?php

if(!defined('BASEPATH'))
    exit('No direct script access allowed');

/**
 * SLA CMS
 *
 * An open source content management system
 *
 * Copyright (c) 2016, Astian Foundation.
 *
 * Astian Develop Public License (ADPL)
 * 
 * This Source Code Form is subject to the terms of the Astian Develop Public
 * License, v. 1.0. If a copy of the APL was not distributed with this
 * file, You can obtain one at http://astian.org/about-ADPL
 * 
 * @author	SHINEARLY
 * @copyright   Copyright (c) 2016, Astian Foundation.
 * @license	http://astian.org/about-ADPL	ADPL License
 * @link	https://www.slacms.com
 * @since	Version 1.0.0
 */
class Navigation extends CI_Controller{

    var $all_lang;

    function __construct(){
        parent::__construct();
        define('LANG', $this->Sla_admin_model->getLang());
        $this->lang->load('admin', LANG);
        $this->template->set_template('admin');
        $this->_init();
    }

    public function _init(){
        $row = $this->Sla_admin_model->load_config();
        $totSegments = $this->uri->total_segments();
        $pageURL = $this->Sla_admin_model->getCurPages();
        $this->template->set('core_css', $this->Sla_admin_model->coreCss());
        $this->template->set('core_js', $this->Sla_admin_model->coreJs());
        $this->template->set('title', 'Backend System | '.$row->site_name);
        $this->template->set('meta_tags', $this->Sla_admin_model->coreMetatags('Backend System for SLA Content Management'));
        $this->template->set('cur_page', $pageURL);
        $this->all_lang = $this->Sla_model->loadAllLang();
    }
    
    private function getPosition(){
        $position = array(
            0 => $this->lang->line('navpage_position_top'),
            1 => $this->lang->line('navpage_position_bottom')
        );
        return $position;
    }

    public function index(){
        admin_helper::is_logged_in($this->session->userdata('admin_email'));
        admin_helper::is_allowchk('navigation');
        $this->db->cache_on();
        $this->sla_referrer->setIndex();
        $this->template->set('cur_page', $this->uri->segment(2));
        if(!$this->uri->segment(3)){
            $lang = $this->Sla_model->getDefualtLang();
        }else{
            $lang = $this->uri->segment(3);
        }
        //Get menu from database
        $this->template->setSub('cur_lang', $lang);
        $this->template->setSub('position', $this->getPosition());
        $this->template->setSub('lang', $this->all_lang);
        $this->load->helper('form');
        //Load the view
        $this->template->loadSub('admin/nav_index');
    }

    public function saveNav(){
        admin_helper::is_logged_in($this->session->userdata('admin_email'));
        admin_helper::is_allowchk('navigation');
        admin_helper::is_allowchk('save');
        $this->Sla_admin_model->sortNav();
        $this->db->cache_delete_all();
        $this->Sla_model->clear_file_cache('topmenu_*', TRUE);
        $this->session->set_flashdata('error_message', '<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
        redirect($this->sla_referrer->getIndex(), 'refresh');
    }

    public function newNav(){
        admin_helper::is_logged_in($this->session->userdata('admin_email'));
        admin_helper::is_allowchk('navigation');
        //Get pages from database
        $this->template->setSub('pages', $this->Sla_admin_model->getPagesAll());
        $this->template->setSub('dropmenu', $this->Sla_admin_model->getDropMenuAll());
        $this->template->setSub('lang', $this->Sla_model->loadAllLang());
        $this->template->setSub('plugin', $this->Sla_admin_model->getPluginAll());
        $this->template->setSub('position', $this->getPosition());
        $this->load->helper('form');
        //Load the view
        $this->template->loadSub('admin/nav_add');
    }

    public function insert(){
        admin_helper::is_logged_in($this->session->userdata('admin_email'));
        admin_helper::is_allowchk('navigation');
        admin_helper::is_allowchk('save');
        //Load the form validation library
        $this->load->library('form_validation');

        $this->form_validation->set_rules('name', $this->lang->line('navpage_menuname'), 'trim|required');

        if($this->form_validation->run() == FALSE){
            //Validation failed
            $this->newNav();
        }else{
            //Validation passed
            $this->Sla_admin_model->insertMenu();
            //Return to navigation list
            $this->db->cache_delete_all();
            $this->Sla_model->clear_file_cache('topmenu_*', TRUE);
            $this->session->set_flashdata('error_message', '<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
            redirect($this->sla_referrer->getIndex(), 'refresh');
        }
    }

    public function editNav(){
        admin_helper::is_logged_in($this->session->userdata('admin_email'));
        admin_helper::is_allowchk('navigation');
        if($this->uri->segment(4)){
            //Get pages from database
            $this->db->cache_on();
            $nav = $this->Sla_model->getValue('*', 'page_menu', 'page_menu_id', $this->uri->segment(4), 1);
            if($nav !== FALSE){
                $this->template->setSub('pages', $this->Sla_admin_model->getPagesAll());
                $this->template->setSub('dropmenu', $this->Sla_admin_model->getDropMenuAll());
                $this->template->setSub('lang', $this->all_lang);
                //Get navigation from database
                $this->template->setSub('nav', $nav);
                $this->template->setSub('plugin', $this->Sla_admin_model->getPluginAll());
                $this->template->setSub('position', $this->getPosition());
                $this->load->helper('form');
                //Load the view
                $this->template->loadSub('admin/nav_edit');
            }else{
                redirect($this->sla_referrer->getIndex(), 'refresh');
            }
        }else{
            redirect($this->sla_referrer->getIndex(), 'refresh');
        }
    }

    public function update(){
        admin_helper::is_logged_in($this->session->userdata('admin_email'));
        admin_helper::is_allowchk('navigation');
        admin_helper::is_allowchk('save');
        //Load the form validation library
        $this->load->library('form_validation');

        $this->form_validation->set_rules('name', $this->lang->line('navpage_menuname'), 'trim|required');

        if($this->form_validation->run() == FALSE){
            //Validation failed
            $this->editNav();
        }else{
            //Validation passed
            $this->Sla_admin_model->updateMenu($this->uri->segment(4));
            //Return to navigation list
            $this->db->cache_delete_all();
            $this->Sla_model->clear_file_cache('topmenu_*', TRUE);
            $this->session->set_flashdata('error_message', '<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
            redirect($this->sla_referrer->getIndex(), 'refresh');
        }
    }

    public function deleteNav(){
        admin_helper::is_logged_in($this->session->userdata('admin_email'));
        admin_helper::is_allowchk('navigation');
        admin_helper::is_allowchk('delete');
        if($this->uri->segment(4)){
            //Delete the user account
            $this->Sla_admin_model->removeData('page_menu', 'page_menu_id', $this->uri->segment(4));
            $this->Sla_admin_model->removeData('page_menu', 'drop_page_menu_id', $this->uri->segment(4));
        }
        //Return to user list
        $this->db->cache_delete_all();
        $this->Sla_model->clear_file_cache('topmenu_*', TRUE);
        $this->session->set_flashdata('error_message', '<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
        redirect($this->sla_referrer->getIndex(), 'refresh');
    }

}
