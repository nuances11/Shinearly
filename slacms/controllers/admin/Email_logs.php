...
class Email_logs extends CI_Controller {
    ...
    public function index() {
        ...
        $result_per_page = 20;
        $total_row = $this->Sla_admin_model->countTable('email_logs', $search_arr);
        $num_link = 10;
        $base_url = $this->Sla_model->base_link(). '/admin/emaillogs/';
        // Pageination config
        $this->Sla_admin_model->pageSetting($base_url,$total_row,$result_per_page,$num_link); 
        ($this->uri->segment(3))? $pagination = ($this->uri->segment(3)) : $pagination = 0;
        //Get users from database
        $this->template->setSub('email_logs', $this->Sla_admin_model->getIndexData('email_logs', $result_per_page, $pagination, 'timestamp_create', 'desc', $search_arr));
    ...
    public function deleteEmailLogs() {
        ...
        if(isset($delR)){
            foreach ($delR as $value) {
                if ($value) {
                    $this->Sla_admin_model->removeData('email_logs', 'email_logs_id', $value);
                }
            }
        }
