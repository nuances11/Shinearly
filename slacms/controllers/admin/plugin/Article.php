...
class Article extends CI_Controller {
    ...
    public function index() {
        ...
        $this->sla_referrer->setIndex('article'); /* Set index page when redirect after save */
        $this->db->cache_on();
        //Get users from database
        $this->template->setSub('total_cat', $this->Sla_model->countData('article_db', "active = '1' AND is_category = '1'"));
        $this->template->setSub('total_art', $this->Sla_model->countData('article_db', "active = '1' AND is_category = '0'"));
    ...
    public function article() {
        admin_helper::is_logged_in($this->session->userdata('admin_email'));
        admin_helper::is_allowchk('article');
        $this->sla_referrer->setIndex('article_art'); /* Set index page when redirect after save */
        $this->db->cache_on();
        $search_arr = ' 1=1 ';
        ...
        $num_link = 10;
        $base_url = $this->Sla_model->base_link(). '/admin/plugin/article/article/';
        ...
        $this->template->setSub('category', $this->Sla_model->getValueArray('*', 'article_db', "is_category", '1'));
    ...
    public function category() {
        admin_helper::is_logged_in($this->session->userdata('admin_email'));
        admin_helper::is_allowchk('article');
        $this->sla_referrer->setIndex('article_cat'); /* Set index page when redirect after save */
        $this->db->cache_on();
        ...
        //Get users from database
        $this->template->setSub('category', $this->Sla_model->getValueArray('*', 'article_db', $search_arr, '', 0, 'arrange', 'asc'));
        $this->template->setSub('main_category', $this->Sla_model->getValueArray('*', 'article_db', "is_category = '1' AND main_cat_id = ''", ''));
        $this->template->setSub('total_row', $total_row);
        $this->template->setSub('lang', $this->Sla_model->loadAllLang());
    ...
    public function catIndexSave(){
        ...
        redirect($this->sla_referrer->getIndex('article_cat'), 'refresh');
    }
    ...
    public function catadd() {
        ...
        //Load the form helper
        $this->load->helper('form');
        $this->template->setSub('category', $this->Sla_model->getValueArray('*', 'article_db', "is_category = '1' AND main_cat_id = ''", ''));
        $this->template->setSub('lang', $this->Sla_model->loadAllLang());
        //Load the view
    ...
    public function addSave() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Sla_model->clear_file_cache('article_getWidget_*', TRUE);
            $this->db->cache_delete_all();
            redirect($this->sla_referrer->getIndex('article_art'), 'refresh');
        }
    }
    ...
    public function artedit() {
        ...
        if ($this->uri->segment(5)) {
            $this->template->setSub('category', $this->Sla_model->getValueArray('*', 'article_db', "is_category", '1'));
            $this->template->setSub('article', $this->Sla_model->getValue('*', 'article_db', 'article_db_id', $this->uri->segment(5), 1));
            $this->template->setSub('lang', $this->Sla_model->loadAllLang());
            //Load the view
    ...
    public function editArtSave() {
        ...
        if ($this->uri->segment(5)) {
            ...
            redirect($this->sla_referrer->getIndex('article'), 'refresh');
        }
    }
    ...
    public function catedit() {
        ...
        if ($this->uri->segment(5)) {
            $this->template->setSub('main_category', $this->Sla_model->getValueArray('*', 'article_db', "is_category = '1' AND main_cat_id = ''", ''));
            $this->template->setSub('category', $this->Sla_model->getValue('*', 'article_db', 'article_db_id', $this->uri->segment(5), 1));
            $this->template->setSub('lang', $this->Sla_model->loadAllLang());
            //Load the view
    ...
    public function asCopy() {
        ...
        admin_helper::is_allowchk('save');
        if($this->uri->segment(5)){
            $article = $this->Sla_model->getValue('*', 'article_db', "article_db_id = '".$this->uri->segment(5)."' AND is_category = '0'", '', 1);
            if($article !== FALSE){
                $data = array(
                    'main_picture' => '',
                ...
                $this->Sla_model->insertAsCopy('article_db', $data);
                $this->db->cache_delete_all();
            }
        }
        redirect($this->sla_referrer->getIndex('article_art'), 'refresh');
