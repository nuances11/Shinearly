...
class Languages extends CI_Controller {
    ...
    public function index() {
        ...
        $total_row = $this->Sla_model->countData('lang_iso');
        //Get users from database
        $this->template->setSub('lang', $this->Sla_model->getValueArray('*', 'lang_iso', '', '', 0, 'arrange', 'asc'));
    ...
    public function insert() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //Validation passed
            //Add the user
            $this->Sla_admin_model->insertLang();
            //Return to user list
            $this->db->cache_delete_all();
            $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
            redirect($this->sla_referrer->getIndex(), 'refresh');
    ...
    public function editLang() {
        ...
        if($this->uri->segment(4)){
            $this->db->cache_on();
            $lang = $this->Sla_model->getValue('*', 'lang_iso', 'lang_iso_id', $this->uri->segment(4), 1);
            if($lang !== FALSE){
    ...
    public function edited() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //Validation passed
            //Update the user
            $this->Sla_admin_model->updateLang($this->uri->segment(4));
            //Return to user list
            $this->db->cache_delete_all();
            $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
            redirect($this->sla_referrer->getIndex(), 'refresh');
    ...
    public function delete() {
        ...
        if($this->uri->segment(4)){
            ...
            if($this->uri->segment(4) != 1) {
                $lang = $this->Sla_model->getValue('lang_iso', 'lang_iso', 'lang_iso_id', $this->uri->segment(4), 1);
                $this->Sla_admin_model->findLangDataUpdate($lang->lang_iso);
                $this->Sla_admin_model->removeData('lang_iso', 'lang_iso_id', $this->uri->segment(4));
                $this->db->cache_delete_all();
                $this->session->set_flashdata('error_message','<div class="alert alert-success" role="alert">'.$this->lang->line('success_message_alert').'</div>');
            } else {
