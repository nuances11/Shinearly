...
class Formsaction extends CI_Controller {
    ...
    function __construct() {
        ...
        if($this->Sla_model->load_config()->maintenance_active){
            //Return to home page
            redirect('./', 'refresh');
            exit;
        }
    ...
    public function index() {
        ...
        if ($form_id) {
            //Get form data
            $frm_rs = $this->Sla_model->getValue('*', 'form_main', 'form_main_id', $form_id, 1);
            if ($frm_rs !== FALSE && $frm_rs->active) {
                $cur_page = $this->session->userdata('slafrm_cururl');
                $field_rs = $this->Sla_model->getValue('*', 'form_field', 'form_main_id', $form_id);
                if ($frm_rs->captcha) {
                    ...
                    if ($f_val->field_type != 'button' && $f_val->field_type != 'reset' && $f_val->field_type != 'submit' && $f_val->field_type != 'label') {      
                        if($frm_rs->form_method == 'post'){
                            if($f_val->field_type == 'email'){
                                $data[$f_val->field_name] = $this->Sla_model->cleanEmailFormat($this->input->post($f_val->field_name, TRUE));
                            }else{
                                $data[$f_val->field_name] = $this->input->post($f_val->field_name, TRUE);
                            }
                        }elseif($frm_rs->form_method == 'get'){
                            if($f_val->field_type == 'email'){
                                $data[$f_val->field_name] = $this->Sla_model->cleanEmailFormat($this->input->get($f_val->field_name, TRUE));
                            }else{
                ...
                if($frm_rs->send_to_visitor && $frm_rs->email_field_id){
                    $visit_field = $this->Sla_model->getValue('field_name', 'form_field', 'form_field_id', $frm_rs->email_field_id, 1);
                    if($visit_field->field_name && $frm_rs->visitor_subject){
                        if($frm_rs->form_method == 'post'){
                            $visit_email = $this->Sla_model->cleanEmailFormat($this->input->post($visit_field->field_name, TRUE));
                        }elseif($frm_rs->form_method == 'get'){
                            $visit_email = $this->Sla_model->cleanEmailFormat($this->input->get($visit_field->field_name, TRUE));
                        }
                        $webconfig = $this->Sla_admin_model->load_config();
                        @$this->Sla_model->sendEmail($visit_email, $frm_rs->visitor_subject, $frm_rs->visitor_body, $email_from, $webconfig->site_name);
                    }
                }
    ...
    private function sendMail($active = '', $email_to = '', $email_from = '', $subject = '', $field_val = '', $form_method) {
        if ($active) {
            ...
            if($email_to){
                $to_email = $this->Sla_model->cleanEmailFormat($email_to);
            }else{
                $to_email = $webconfig->default_email;
            }            
            $message_html = $this->Sla_model->getLabelLang('email_dear') . $to_email . ',<br><br>';
            if ($field_val) {
                        ...
                        if($val->field_type == 'textarea'){
                            ...
                            if($form_method == 'post'){
                                if($val->field_type == 'email'){
                                    $message_html.= '<b>' . $field_label . ':</b> ' . $this->Sla_model->cleanEmailFormat($this->input->post($val->field_name, TRUE)) . '<br>';
                                }else{
                                    $message_html.= '<b>' . $field_label . ':</b> ' . $this->input->post($val->field_name, TRUE) . '<br>';
                                }
                            }elseif($form_method == 'get'){
                                if($val->field_type == 'email'){
                                    $message_html.= '<b>' . $field_label . ':</b> ' . $this->Sla_model->cleanEmailFormat($this->input->get($val->field_name, TRUE)) . '<br>';
                                }else{
            ...
            $message_html.= '<br><br>' . $this->Sla_model->getLabelLang('email_footer') . ' <br><a href="' . $this->Sla_model->base_link(). '" target="_blank"><b>' . $webconfig->site_name . '</b></a>';           
