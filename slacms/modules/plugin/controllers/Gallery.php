...
class Gallery extends CI_Controller {
    ...
    public function _init() {
        $this->template->set('core_css', $this->Sla_model->coreCss('assets/css/ekko-lightbox.min.css'));
        $js_arr = array(base_url() . 'assets/js/ekko-lightbox.min.js', base_url() . 'assets/js/ekko-lightbox.run.js');
        $this->template->set('core_js', $this->Sla_model->coreJs($js_arr));
        $row = $this->Sla_model->load_config();
    ...
    public function index() {
        $row = $this->Sla_model->load_config();
        $title = $this->Sla_model->getLabelLang('gallery_header') . ' | ' . $row->site_name;
        ...
        // Pages variable
        $result_per_page = 15;
        $total_row = $this->Sla_model->countData('gallery_db', $search_arr);
        $num_link = 10;
        $base_url = $this->Sla_model->base_link(). '/plugin/gallery/';
        ...
        $this->template->setSub('gallery', $this->Sla_admin_model->getIndexData('gallery_db', $result_per_page, $pagination, 'arrange', 'asc', $search_arr));
    ...
    public function view() {
        if($this->uri->segment(4)){
            $album_row = $this->Sla_model->getValue('*', 'gallery_db', "active = '1' AND gallery_db_id = '".$this->uri->segment(4)."'", '', 1);
            if($album_row !== FALSE){
                if($this->session->userdata('fronlang_iso') != $album_row->lang_iso){
                    $this->Sla_model->setSiteLang($album_row->lang_iso);
                }
                $row = $this->Sla_model->load_config();
                $title = $album_row->album_name.' | ' . $row->site_name;
                $this->template->set('title', $title);
                $this->template->set('meta_tags', $this->Sla_model->coreMetatags($album_row->short_desc,$album_row->keyword,$title,$this->Gallery_model->getFirstImgs($album_row->gallery_db_id)));
                $this->template->set('cur_page', $this->page_url);
                ...
                $result_per_page = 30;
                $total_row = $this->Sla_model->countData('gallery_picture', $search_arr);
                $num_link = 10;
                $base_url = $this->Sla_model->base_link(). '/plugin/gallery/view/'.$this->uri->segment(4).'/'.$album_row->url_rewrite.'/';

                // Pageination config
                $this->Sla_admin_model->pageSetting($base_url, $total_row, $result_per_page, $num_link, 6);
                ($this->uri->segment(6)) ? $pagination = $this->uri->segment(6) : $pagination = 0;

                //Get users from database
                $this->template->setSub('image', $this->Sla_admin_model->getIndexData('gallery_picture', $result_per_page, $pagination, 'arrange', 'asc', $search_arr));
                $this->template->setSub('total_row', $total_row);

                //Load the view
                $this->template->loadSub('gallery/gallery_view');
            }else{
                redirect($this->Sla_model->base_link().'/plugin/gallery', 'refresh');
            }
        }else{
            redirect($this->Sla_model->base_link().'/plugin/gallery', 'refresh');
        }
    ...
    public function rss() {
        ...
        $feed->description = $this->Sla_model->getLabelLang('gallery_header') . ' | ' . $row->site_name;
        $feed->link = $this->Sla_model->base_link().'/plugin/gallery';
        $search_arr = " active = '1'";
        $limit = 20;
        // get article list
        $gallery = $this->Sla_admin_model->getIndexData('gallery_db', $limit, 0, 'arrange', 'asc', $search_arr);
        // add posts to the feed
        if($gallery !== FALSE){
            foreach ($gallery as $a)
            {
                // set item's title, author, url, pubdate and description
                $url = $this->Sla_model->base_link().'/plugin/gallery/view/'.$a['gallery_db_id'].'/'.$a['url_rewrite'];
    ...
    public function getWidget() {
        ...
        if (!$this->cache->get('gallery_getWidget_'.$this->uri->segment(4))) {
            ...
            $xml->addNode('main_url', $this->Sla_model->base_link().'/plugin/gallery');
            ...
            $article = $this->Sla_admin_model->getIndexData('gallery_db', 100, 0, 'arrange', 'asc', $search_arr);
            if($article !== FALSE){
                $xml->addNode('null', '0'); // For check item is not empty
                foreach ($article as $row) {
                    // start sub branch
                    $xml->startBranch('item', array('id' => $row['gallery_db_id'])); 
                    $xml->addNode('sub_url', $this->Sla_model->base_link().'/plugin/gallery/view/'.$row['gallery_db_id'].'/'.$row['url_rewrite']);
