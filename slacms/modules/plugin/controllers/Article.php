...
class Article extends CI_Controller {
    ...
    public function index() {
        $row = $this->Sla_model->load_config();
        $title = $this->Sla_model->getLabelLang('article_index_header') . ' | ' . $row->site_name;
        ...
        // Pages variable
        $result_per_page = 10;
        $total_row = $this->Sla_model->countData('article_db', $search_arr);
        $num_link = 10;
        $base_url = $this->Sla_model->base_link(). '/plugin/article/';

        // Pageination config
        $this->Sla_admin_model->pageSetting($base_url, $total_row, $result_per_page, $num_link, 3);
        ($this->uri->segment(3)) ? $pagination = $this->uri->segment(3) : $pagination = 0;

        //Get users from database
        $this->template->setSub('article', $this->Sla_admin_model->getIndexData('article_db', $result_per_page, $pagination, 'timestamp_create', 'desc', $search_arr));
    ...
    public function category() {
        if($this->uri->segment(4)){
            $cat_row = $this->Sla_model->getValue('*', 'article_db', "is_category = '1' AND active = '1' AND url_rewrite = '".$this->uri->segment(4)."'", '', 1);
            if($cat_row !== FALSE){
                if($this->session->userdata('fronlang_iso') != $cat_row->lang_iso){
                    $this->Sla_model->setSiteLang($cat_row->lang_iso);
                }
                $row = $this->Sla_model->load_config();
                $title = $this->Sla_model->getLabelLang('article_index_header') . ' | ' . $row->site_name;
                $this->template->set('title', $title);
                ...
                $total_row = $this->Sla_model->countData('article_db', $search_arr);
                $num_link = 10;
                $base_url = $this->Sla_model->base_link(). '/plugin/article/category/'.$this->uri->segment(4).'/';

                // Pageination config
                $this->Sla_admin_model->pageSetting($base_url, $total_row, $result_per_page, $num_link, 5);
                ...
                redirect($this->Sla_model->base_link().'/plugin/article', 'refresh');
            }
        }else{
            redirect($this->Sla_model->base_link().'/plugin/article', 'refresh');
        }
    ...
    public function search() {
        $p = $this->Sla_model->cleanOSCommand($this->input->get('p', TRUE));
        if ($p) {
            $row = $this->Sla_model->load_config();
            ...
            $total_row = $this->Sla_model->countData('article_db', $search_arr);
            $num_link = 10;
            $base_url = $this->Sla_model->base_link(). '/plugin/article/search/';

            // Pageination config
            $this->Sla_admin_model->pageSetting($base_url, $total_row, $result_per_page, $num_link, 4);
    ...
    public function archive() {
        if($this->uri->segment(4)){
            ...
            if($year_arr !== FALSE){
                ...
                $total_row = $this->Sla_model->countData('article_db', $search_arr);
                $num_link = 10;
                $base_url = $this->Sla_model->base_link(). '/plugin/article/archive/'.$this->uri->segment(4).'/';
    ...
    public function view() {
        if($this->uri->segment(4) && $this->uri->segment(5)){
            $art_row = $this->Sla_model->getValue('*', 'article_db', "is_category = '0' AND active = '1' AND article_db_id = '".$this->uri->segment(4)."' AND url_rewrite = '".$this->uri->segment(5)."'", '', 1);
            if($art_row !== FALSE){
                if($this->session->userdata('fronlang_iso') != $art_row->lang_iso){
                    $this->Sla_model->setSiteLang($art_row->lang_iso);
                }
                $row = $this->Sla_model->load_config();
                if ($this->Sla_model->findFrmTag($art_row->content) !== false) {
                ...
                $this->template->set('title', $title);
                $more_meta = '<link rel="amphtml" href="'.$this->Sla_model->base_link().'/plugin/article/amp/'.$this->uri->segment(4).'/'.$this->uri->segment(5).'">' . "\n";
                ...
                $facebook = $this->Sla_model->getValue('*', 'footer_social', "social_name = 'facebook' AND active = '1'", '', 1);
                if($facebook !== FALSE && $facebook->social_url){
                    $more_meta.= '<meta property="article:author" content="' . $facebook->social_url . '" />' . "\n";
                }
                $this->template->set('meta_tags', $this->Sla_model->coreMetatags($art_row->short_desc,$art_row->keyword,$title,'plugin/article/' . $art_row->main_picture,$more_meta));
                $this->template->set('cur_page', $this->page_url);

                //Get users from database
                $this->template->setSub('article', $art_row);
                $cat_row = $this->Sla_model->getValue('category_name', 'article_db', "is_category = '1' AND active = '1' AND article_db_id = '".$art_row->cat_id."'", '', 1);
                $this->template->setSub('category_name', $cat_row->category_name);
    ...
    public function amp() {
        if($this->uri->segment(4) && $this->uri->segment(5)){
            ...
            if($art_row !== FALSE){
                ...
                $title = $art_row->title.' | ' . $row->site_name;
                $this->template->set('title', $title);
                $canonical = '<link rel="canonical" href="'.$this->Sla_model->base_link().'/plugin/article/view/'.$this->uri->segment(4).'/'.$this->uri->segment(5).'">' . "\n";               
    ...
    public function rss() {
        ...
        $feed->description = $this->Sla_model->getLabelLang('article_index_header') . ' | ' . $row->site_name;
        $feed->link = $this->Sla_model->base_link().'/plugin/article';
        $search_arr = " is_category = '0' AND active = '1'";
        $limit = 20;
        // get article list
        $article = $this->Sla_admin_model->getIndexData('article_db', $limit, 0, 'timestamp_create', 'desc', $search_arr);
        // add posts to the feed
        if($article !== FALSE){
            foreach ($article as $a)
            {
                // set item's title, author, url, pubdate and description
                $url = $this->Sla_model->base_link().'/plugin/article/view/'.$a['article_db_id'].'/'.$a['url_rewrite'];
    ...
    public function getWidget() {
        ...
        if (!$this->cache->get('article_getWidget_'.$this->uri->segment(4).'_'.$this->uri->segment(5))) {
            ...
            // Initiate class
            $xml = new Xml_writer;
            $xml->setRootName('sla_widget');
            $xml->initiate();
            // Start Main branch
            $xml->startBranch('plugin'); 
            $xml->addNode('main_url', $this->Sla_model->base_link().'/plugin/article');
            ...
            $article = $this->Sla_admin_model->getIndexData('article_db', 100, 0, 'timestamp_create', 'desc', $search_arr);
            if($article !== FALSE){
                $xml->addNode('null', '0'); // For check item is not empty
                foreach ($article as $row) {
                    // start sub branch
                    $xml->startBranch('item', array('id' => $row['article_db_id'])); 
                    $xml->addNode('sub_url', $this->Sla_model->base_link().'/plugin/article/view/'.$row['article_db_id'].'/'.$row['url_rewrite']);
                    $xml->addNode('title', $row['title']);
                    $xml->addNode('short_desc', $row['short_desc']);
                    if($row['main_picture']){
                        $xml->addNode('photo', $this->Sla_model->base_link().'/photo/plugin/article/'.$row['main_picture']);
                    }else{
                        $xml->addNode('photo', $this->Sla_model->base_link().'/photo/no_image.png');
                    }
                    // End sub branch
                    $xml->endBranch();
                }
