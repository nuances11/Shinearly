    ...
    */
    static function is_logged_in($email_session){
        ...
        if(!$email_session || !$_SESSION['admin_logged_in']){
            ...
            $sess_data = array('slablogin_cururl' => $url_return);
            $CI->session->set_userdata($sess_data);
            $redirect= $CI->Sla_model->base_link().'/admin/login';
            ...
            if($chk === FALSE){
                $redirect= $CI->Sla_model->base_link().'/admin/logout';
                header("Location: $redirect");	
                exit;
            }
        }
    ...
    */
    static function login_already($email_session){
        if($email_session && $_SESSION['admin_logged_in'] && $_SESSION['session_id']){
            $CI =& get_instance();
            $redirect= $CI->Sla_model->base_link().'/admin';
            header("Location: $redirect");	
            exit;
        }
    }
    ...
    */
    static function is_allowchk($perms_name){
        ...
        if($perms_name){
            ...
            if($CI->Sla_auth_model->is_group_allowed($perms_name, 'backend') === FALSE){
                $CI->load->library('session');
                $CI->load->library('sla_referrer');
                $CI->load->helper('url');
                $CI->session->set_flashdata('error_message','<div class="alert alert-danger" role="alert">'.$CI->lang->line('user_not_allow_txt').'</div>');
                redirect($CI->sla_referrer->getIndex(), 'refresh');
                exit;
            }
            if($perms_name == 'save' || $perms_name == 'delete'){
                $CI->load->model('Sla_admin_model');
                $CI->load->helper('url');
                $CI->Sla_admin_model->saveActionsLogs(current_url(), $perms_name, $perms_name . ' on [' . uri_string() . ']');
            }
        }
    }
